...
byte Read_SV[8] = {0x01,0x03,0x00,0x00,0x00,0x01,0x84,0x0A}  ;  //讀取單筆參數資料：Master送出資料(讀取SV現在資料
//-----------------
  typedef struct Word
  {
      byte HI;
...
{
  int stage = 0 ;
   Serial.println("Enter Get_SV--While") ;
  while (Serial2.available()>0)
  {
...
float GETPV()
{
  long gg = WordValue(&retdata.Data) ;
    return gg/10+0.1*(gg%10) ;
}
